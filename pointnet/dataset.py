    ...
    with open(
        ...
        for item in cat:
            ...
            for i in tqdm(range(len(datapath))):
                l = len(
                    np.unique(np.loadtxt(datapath[i][-1]).astype(np.uint8))
                )
                if l > num_seg_classes:
                    num_seg_classes = l

            print(
                "category {} num segmentation classes {}".format(
        ...
        for file in filelist:
            ...
            if category in self.cat.values():
                self.meta[self.id2cat[category]].append(
                    (
                        os.path.join(
                            self.root, category, "points", uuid + ".pts"
                        ),
                        os.path.join(
...
class ModelNetDataset(data.Dataset):
    def __init__(
        self, root, npoints=2500, split="train", data_augmentation=True
    ):
        self.npoints = npoints
        self.root = root
        self.split = split
